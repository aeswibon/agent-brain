...
- **OpenCode route gate** — recognize `agent-brain_*` tool names (not only `mcp_*`); plugin fails closed when `route_gate.py` is missing or errors; `tool.execute.after` clears gate after successful `route_task`
- **`doctor --fix`** — reinstalls OpenCode integration, Codex hooks, and Gemini hooks when incomplete

### Changed

- **`docs/host-integration.md`** — OpenCode enforcement notes, multi-host reinstall guidance
...
- **agent-body-core dependency** — updated tag to `v0.5.14` for ecosystem alignment with Dual-Mode MCP Gateway release
